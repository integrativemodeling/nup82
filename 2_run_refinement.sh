...
MODELING_SCRIPT=2_modeling_allEM_except11_19.py
XL_FILE=XL.csv
...
if [ $PWD_PARENT != $PWD ]; then
    ...
    $IMP python ./$MODELING_SCRIPT -r $REPEAT -out $OUTPUT -rmf $RMF_FILE -rmf_n $RMF_FRAME -em2d $EM2D_FILE -weight $EM2D_WEIGHT
    cd ..
fi
